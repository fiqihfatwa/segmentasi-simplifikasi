...
public class MainTrainer 
{
	...
	public static void Train(String corpus) {
		...
		List<TaggedWord> startMarkers = new ArrayList<>();
		startMarkers.add(new TaggedWord("<STARTTAG>", "<STARTTAG>"));
		startMarkers.add(new TaggedWord("<STARTTAG>", "<STARTTAG>"));
		List<TaggedWord> endMarkers = new ArrayList<>();
		...
		try 
		{
			corpusReader.parse(new BufferedReader(new FileReader(corpus)));
                        //        JOptionPane.showMessageDialog(null, "hai","Kesalahan",JOptionPane.ERROR_MESSAGE);
                } 
		catch (IOException e) 
		{
		...
		try 
		{
			writeLexicon(trainHandler.getLexicon(), new BufferedWriter(new FileWriter("src\\resource\\Lexicon.trn")));
			writeNGrams(trainHandler.getUnigram(), trainHandler.getBigram(),
				    trainHandler.getTrigram(), trainHandler.getQuatogram(),
				    new BufferedWriter(new FileWriter("src\\resource\\Ngram.trn")));
		}
		catch (IOException e) 
		{
			// Error IO..
